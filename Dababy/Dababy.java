...
public class Dababy extends JFrame implements ActionListener{
    ...
    Dababy(){
        ...
        buttonMaker(custom, 150, 700, 250, 100,"Ully Ah Dag");

        buttonMaker(left, 50, 600, 100, 100,"←");
        buttonMaker(right, 150, 600, 100, 100,"→");
        buttonMaker(up, 100, 500, 100, 100,"↑");
        buttonMaker(down, 100, 700, 100, 100,"↓");
        ...
        daReset = new JButton(new ImageIcon(asset +"reset.png"));
        daReset.setBounds(800, 775, 50, 50);
        daReset.addActionListener(this);
        this.add(daReset);
        daReset.setVisible(false);
    ...
    public int setPerfectMove() {
        perfectMove = 2 + (java.lang.Math.abs(xRand - x))/50 + (java.lang.Math.abs(yRand - y))/50;
        return perfectMove;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getSource() == daReset) {
            daFood.setBounds(getRandomX(), getRandomY(), screenDimension.width, screenDimension.height);
            score = score - 5;
            daClockText.setText("Score : " + score + " Highscore : " + highScore);
            System.out.println(x+","+y+","+xRand+","+yRand);
            changeDirection();
            System.out.println(setPerfectMove());

        }
        if(e.getSource() == clock) {
            daReset.setVisible(true);
        ...
        if(e.getSource() == left) {
            ...
            if ((direction == 0) && (y == yRand) && (x == xRand)) {
                if ((perfectMove - moveCounter) == 0) {
                    score++;
                } else {
                    score = 1 + score + perfectMove - moveCounter;
                }
                daFood.setBounds(getRandomX(), getRandomY(), screenDimension.width, screenDimension.height);
                changeDirection();
                scoreWriter();
    ...
    public void motionControl() {
        ...
        daIcon.setBounds(x, y , screenDimension.width, screenDimension.height);
        c.add(daIcon);
        System.out.println(perfectMove - moveCounter);
